#!/usr/bin/env python3
"""Binance'de acik ama DB'de kayitli olmayan harici pozisyonlari ice aktarir.

Kullanim (sunucuda):
    docker compose exec api python scripts/import_external_positions.py admin@example.com
"""
...
from shared.db import Admin, BotRuntimeStatus, Position
...
async def main() -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if settings.mode == "paper":
            raise SystemExit("PAPER modunda harici pozisyon ice aktarimi yapilamaz.")
        ...
        print(f"Binance acik pozisyon: {len(exchange_positions)}")
        print(f"DB acik pozisyon (once): {len(local_symbols)}")
        print(f"Ice aktarilan harici pozisyon: {len(imported)}")
        if imported:
            print("  ", ", ".join(imported))

        run = await run_and_persist_reconciliation(session, admin.id, settings.mode, triggered_by="import_external")
        print(f"Reconciliation: {run.status}, mismatches={run.mismatches_found}, external={run.external_positions_found}")
        ...
        if runtime is not None:
            ...
            if runtime.safe_mode_reason:
                print(f"Safe mode nedeni: {runtime.safe_mode_reason}")
